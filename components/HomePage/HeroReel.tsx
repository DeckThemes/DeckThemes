...
export function HeroReel() {
  ...
  return (
    <>
      <div className="relative mt-16 h-[24rem] max-w-[calc(100vw-48px)] overflow-hidden sm:mt-24 md:h-[36rem] flex items-center">
        <style>
          {`
            ...
            <>
              {themeData.items.map((data, index) => (
                <Link
                  ...
                  key={index}
                  className={`img-shadow group relative aspect-[16/10] w-[24rem] md:w-[32rem] flex-none rounded-xl border-2 border-borders-base1-light bg-[#27272a] transition dark:border-borders-base1-dark dark:bg-zinc-800 sm:rounded-2xl ${getRandomRotationClass(
                    index
                  )}`}
                >
